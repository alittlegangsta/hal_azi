#!/usr/bin/env python3
"""Smoke-check EXP-008 depth-heldout TFRecord split outputs without training."""
...
def parse_one_record_with_tensorflow(path: Path, expected_feature_shape: tuple[int, ...], expected_label_shape: tuple[int, ...]) -> dict:
    try:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--expected-label-shape", default="70,30")
    ...
    if manifest_path.exists():
        ...
        for split in ("train", "val", "test"):
            if counts.get(split) != manifest_counts.get(split):
                errors.append(
                    f"{split} count mismatch: tfrecord={counts.get(split)} manifest={manifest_counts.get(split)}"
                )
    else:
