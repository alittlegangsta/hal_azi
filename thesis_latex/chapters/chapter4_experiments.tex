...
\begin{enumerate}
  \item 基于 CWT 特征的窜槽二分类可学习性验证实验（内部编号 EXP-006，以下简称实验一）；
  \item 基于一维窜槽占比标签的回归对照实验（内部编号 EXP-007，以下简称实验二）；
  \item 基于方位旋转不变 FFT 幅值标签的窜槽结构特征反演实验（内部编号 EXP-008，以下简称实验三）。
\end{enumerate}
...
\section{Grad-CAM 图件核验}

现有 Grad-CAM 组合图显示，随机样本划分先导模型对部分样本的梯度响应集中在约 0.5--1.2~ms、18--26~kHz 的区域。但图件只能确定来自早期方位 FFT 回归输出，无法唯一核验具体训练运行、checkpoint、回归目标系数和卷积层；其样本编号也不能与本章 423 个深度隔离测试样本建立可靠对应。

基于上述来源限制，本章不插入这些 Grad-CAM 图，也不把先导模型热区解释为实验三的特征响应。Grad-CAM 的方法定义和解释边界已在第 3 章给出\cite{Selvaraju2017GradCAM}。后续只有在模型、划分、checkpoint、目标输出、样本深度和卷积层全部可追溯时，才适合开展正确样本与大误差样本的时频响应比较。

\section{本章小结}

本章在相同单井深度区间隔离划分下比较了两种回归标签，并以随机样本划分二分类作为先导验证。实验三测试 MAE 为 0.079025、RMSE 为 0.277019、$R^2$ 为 0.106634，相关性为中等偏弱，支持有限可学习性而非高精度结论。模型在 RMSE 和 $R^2$ 上优于零值基线，却未在 MAE 上占优；与训练集中位数基线的 RMSE 接近。低阶 FFT 系数和较高严重度样本构成主要误差来源。实验二降低输出维度后 $R^2$ 仍为负，说明深度区间迁移并未因标签简化而解决。来源未能完全核验的 Grad-CAM 图件未用于正文证据。
