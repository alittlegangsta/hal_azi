...
# --- (make_gradcam_heatmap_for_regression 函数保持不变) ---
...
def _calculate_mean_severity(profile):
    """
    (新增辅助函数)
    计算单个样本(1D或FFT)的平均严重性数值，用于绘图和分类。
    """
    if profile.ndim == 2:
        # FFT 任务 (70, 30)
...
def plot_overall_performance_scatter(all_labels, all_predictions, output_path):
    ...
    df = pd.DataFrame({
        'Ground Truth Severity': true_values,
        'Predicted Severity': pred_values,
        'Category': categories
    })
...
def generate_waveform_and_gradcam_plots(model, all_cwts_original, all_cwts_masked, all_waveforms, all_labels, output_dir, num_samples_per_category=8):
    """
    (已修正)
    all_cwts_original: 用于绘图
    all_cwts_masked:   用于 Grad-CAM 计算
    """
    ...
    for category in categories_order:
        ...
        for i in tqdm(indices_to_plot, desc=f"生成 {category} 组合图"):
            ...
            cwt_image_original = all_cwts_original[i] # 用于绘图
            cwt_image_masked = all_cwts_masked[i]     # 用于计算
            
            img_array_expanded = np.expand_dims(cwt_image_masked, axis=0) # 使用掩码后的图像
            ...
            ax2.imshow(cwt_image_original[:, :, 0], aspect='auto', cmap='gray', extent=cwt_plot_extent) # 绘制原始CWT
...
# --- (create_profile_comparison_image 函数保持不变) ---
def create_profile_comparison_image(all_labels, all_predictions, sonic_depths, output_path):
    print(f"\n--- 正在生成虚拟成像对比图 (Task: {TASK_TYPE}) ---")
    
    if all_labels[0].ndim == 2:
        truth_image = np.stack(all_labels, axis=0)[:, :, 0]
        predictions_image = all_predictions[:, :, 0]
        title_suffix = '(FFT Coeff 0)'
        cmap_label = 'Log(1 + Integrated Severity)'
        vmax = 6.5
    else:
        truth_image = np.stack(all_labels, axis=0)
        predictions_image = all_predictions
        title_suffix = '(1D Profile)'
        cmap_label = 'Channeling Percentage (%)'
        vmax = 50

    error_image = predictions_image - truth_image
    
    fig, (ax1, ax2, ax3) = plt.subplots(1, 3, figsize=(20, 15), sharey=True)
    fig.suptitle(f'Virtual Profile Log Comparison ({TASK_TYPE})', fontsize=18)
    ...
    ax1.set_title(f'Ground Truth Profile {title_suffix}', fontsize=14)
    ax1.set_xlabel(f'Relative Profile Depth Points (0-{truth_image.shape[1]})')
    ax1.set_ylabel('Absolute Sonic Depth (ft)')
    fig.colorbar(im1, ax=ax1, label=cmap_label)

    im2 = ax2.imshow(predictions_image, aspect='auto', cmap='jet', extent=plot_extent, vmin=0, vmax=vmax)
    ax2.set_title(f'Predicted Profile {title_suffix}', fontsize=14)
    ax2.set_xlabel(f'Relative Profile Depth Points (0-{truth_image.shape[1]})')
    fig.colorbar(im2, ax=ax2, label=cmap_label)

    error_limit = vmax / 2.5
    ...
    ax3.set_xlabel(f'Relative Profile Depth Points (0-{truth_image.shape[1]})')
...
def run_analysis_regressor():
    ...
    with h5py.File(cwt_h5_path, 'r') as cwt_file:
        all_cwts_original = [cwt_file['cwt_images'][i] for i in range(len(all_labels))] # 重命名
